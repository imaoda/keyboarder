...
exports.default = addKeyListener;

function addKeyListener(obj, param2, param3) {
  var delay = 1500;
  ...
  function process(e) {
    if (e.target.tagName.match(/(textarea|input)/i)) return; // 干掉之前的 sto

    if (sto) clearTimeout(sto); // 如果点击了 esc 键，清空
    ...
    if (isSpecialKey(keyArr[0].key) && keyArr.length > 1) {
      ...
      }).join("|"); // 特殊规则，对那种按了一次 specail 键并松开，再按任意1键的情形

      if (search.length === 1) search = "|" + search;
    } else if (!isSpecialKey(keyArr[0].key)) {
      search = keyArr.map(function (e) {
        return e.key;
      }).join("").toLowerCase();
    }

    cb(search); // 如果没有命中，则继续等待（不考虑那种一定错了的）

    if (!obj[search]) {
      sto = setTimeout(function () {
        keyArr = [];
      }, delay);
    } else if (cmds.filter(function (cmd) {
      return cmd.indexOf(search) === 0;
    }).length > 1) {
      // 命中了，并且还有待匹配的，那么延迟执行
      sto = setTimeout(function () {
        keyArr = [];
